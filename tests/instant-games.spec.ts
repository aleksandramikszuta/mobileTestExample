...
test('User can see “ZEAL Instant Games” teaser section and can swipe it', async ({ page }) => {
    mainPage = new MainPage(page);
    detailsPage = new DetailsPage(page)
    helpPage = new HelpPage(page)

    await page.goto('https://games.lotto24.de');
    await mainPage.acceptAllCookies();
    await expect(mainPage.zealTeaser.locator(mainPage.headline)).toContainText('ZEAL Instant Games');
    await swipeFromElement(page, mainPage.zealTeaser, -200);
    await swipeFromElement(page, mainPage.zealTeaser, -200);
    const gameName = await mainPage.getGameName(mainPage.singleGameTeaser)
    const url = 'https://games.lotto24.de/games/' + gameName.toLowerCase().replace(' ','');
